...
public class JSONController {
	 ...
	 * @return the webshops
	 */
	@Get(value = "/find/q{?queryParams}", produces = "application/json", consumes = "application/json")
	public HttpResponse<WebshopModel[]> getWebshops(String queryParams) {
		Condition[] conditions = queryParamParser.parseParams(queryParamParser.parseQuery(queryParams)).toArray(Condition[]::new);
		List<Webshop> webshops;

		webshops = webshopRepository.findVarious(conditions).collectList().block();
